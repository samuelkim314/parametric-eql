...
from utils.conv_relu_network import ReLUConvNet
from utils.schedules import get_lr
...
RES_DIRECTORY = "benchmark-relu-conv/"
...
class Benchmark(benchmark.Benchmark):
    ...
    def __init__(self, results_dir, numW=2, n_layers=2, learning_rate=1e-2,
    ...
    def benchmark(self, func, func_name, trials, train_data, test_data):
        ...
        # List of (error, expr)
        idx_list = [i for i in range(trials)]
        error_expr_sorted = sorted(zip(error_test_list, idx_list))
        # Separating out the errors
        error_test_sorted = [x for x, _ in error_expr_sorted]
        idx_list = [x for _, x in error_expr_sorted]

        fi = open(os.path.join(self.results_dir, 'eq_summary.txt'), 'a')
        fi.write("\n{}\n".format(func_name))
        for i in range(trials):
            fi.write("[%f]\t\t%d\n" %
                     (error_test_sorted[i], idx_list[i]))
        fi.close()
                    ...
                    for i in range(self.n_epochs1):
                        ...
                        for k in range(len(x_batches)):
                            feed_dict = {t_batch_placeholder: t_batches[k], x_placeholder: x_batches[k], y_placeholder: y_batches[k],
                                         y_max_placeholder: y_max_batches[k], y_min_placeholder: y_min_batches[k], y_range_placeholder: y_range_batches[k], learning_rate: get_lr(self.learning_rate, i, self.n_epochs1)}
                            _ = sess.run(train, feed_dict=feed_dict)

                        if i % self.summary_step == 0:
                            loss_val, error_val = sess.run(
                                (loss, error), feed_dict=feed_dict)
