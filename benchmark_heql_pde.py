...
from utils.para_sym_network import HyperEQL
from utils.schedules import get_lr, get_rw
from inspect import signature
import benchmark
import argparse

from generate_data.addiff import in_data as addiff_in_data, out_data as addiff_out_data, t_data as addiff_t_data
from generate_data.burgers import in_data as burgers_in_data, out_data as burgers_out_data, t_data as burgers_t_data
from generate_data.ks import in_data as ks_in_data, out_data as ks_out_data, t_data as ks_t_data
...
var_names = ["x", "y", "z", "a"]
var_names = [f"u{i}" for i in range(20)]
...
RES_DIRECTORY = "benchmark-heql-pde/"
...
class Benchmark(benchmark.Benchmark):
    ...
    def benchmark(self, xdim, func_name, trials, train_data, test_data):
        ...
        expr_list, error_test_list = self.train(
            xdim, train_data, test_data, func_name, trials, func_dir)
    ...
    def train(self, x_dim, train_data, test_data, func_name='', trials=1, func_dir='results/test'):
        ...
        sym = HyperEQL(self.n_layers, funcs=self.activation_funcs)
        y_hat = sym(t_batch_placeholder, x_placeholder)
        ...
        with tf.Session(config=config) as sess:
            for trial in range(trials):
                ...
                # Save data
                y_hat_norm = sess.run(y_hat, feed_dict={t_batch_placeholder: t_batch_test, x_placeholder: x_test})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n-epochs1", type=int, default=2401,
    ...
    bench.benchmark(3, func_name="addiff", trials=NUM_TRIALS, train_data=(addiff_in_data, addiff_out_data, addiff_t_data),
                    test_data=(addiff_in_data, addiff_out_data, addiff_t_data))
    bench.benchmark(3, func_name="burgers", trials=NUM_TRIALS, train_data=(burgers_in_data, burgers_out_data, burgers_t_data),
                    test_data=(burgers_in_data, burgers_out_data, burgers_t_data))
    bench.benchmark(4, func_name="ks", trials=NUM_TRIALS, train_data=(ks_in_data, ks_out_data, ks_t_data),
                    test_data=(ks_in_data, ks_out_data, ks_t_data))
