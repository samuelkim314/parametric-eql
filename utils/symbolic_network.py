...
GAMMA = -0.1
ZETA = 1.1
# BETA = [1/3, 1/2, 2/3, 5/6]
# GAMMA = [-0.05, -0.1, -0.2, -0.4]
# ZETA = [1.05, 1.1, 1.2, 1.4]
EPSILON = 1e-6
...
class SymbolicLayer:
    ...
    def __init__(self, funcs=None, initial_weight=None, variable=False, init_stddev=0.1):
        ...
        # Number of activation functions (and number of layer outputs)
        self.n_funcs = len(funcs)
        # Convert functions to list of Tensorflow functions
        self.funcs = [func.tf for func in funcs]
        # Number of activation functions that take 2 inputs
        self.n_double = functions.count_double(funcs)
        # Number of activation functions that take 1 input
    ...
    def build(self, in_dim):
        ...
        self.W = tf.Variable(tf.random_normal(
            shape=[in_dim, self.out_dim], stddev=self.init_stddev))
        self.built = True
    ...
    def __init__(self, symbolic_depth, funcs=None, initial_weights=None, initial_bias=None,
        ...
        if initial_weights is not None:
            ...
            if isinstance(init_stddev, list):
                self.symbolic_layers = [SymbolicLayer(
                    funcs=funcs, init_stddev=init_stddev[i]) for i in range(self.depth)]
            else:
                self.symbolic_layers = [SymbolicLayer(
                    funcs=funcs, init_stddev=init_stddev) for _ in range(self.depth)]
            # Initialize weights for last layer (without activation functions)
            self.output_weight = tf.Variable(tf.random_uniform(
                shape=(self.symbolic_layers[-1].n_funcs, 1)))
...
class MaskedSymbolicNet(SymbolicNet):
    ...
    def __init__(self, sess, sr_unit, threshold=0.01):
        ...
        for w_i in weights:
            mask = tf.constant(
                sess.run(tf.abs(w_i) > threshold), dtype=tf.float32)
            masked_weights.append(tf.multiply(w_i, mask))

        super().__init__(sr_unit.depth, funcs=sr_unit.funcs,
                         initial_weights=masked_weights, variable=True)
...
class SymbolicLayerL0(SymbolicLayer):
    ...
    def sample_u(self, shape, reuse_u=False):
        ...
        if self.eps is None or not reuse_u:
            self.eps = tf.random.uniform(
                shape=shape, minval=EPSILON, maxval=1.0 - EPSILON)
        return self.eps
...
class SymbolicCell(tf.keras.layers.SimpleRNNCell):
    ...
    def call(self, inputs, state, training=None):
        ...
        output = tf.concat(
            [self.sym1(full_input), self.sym2(full_input)], axis=1)
        next_state = output
