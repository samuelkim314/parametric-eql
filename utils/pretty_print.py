...
def sym_pp(W_list, funcs, var_names, threshold=1e-5, n_double=0, skip_connections=True):
    ...
    if not skip_connections:
        ...
        for i in range(len(W_list)):
            ...
            if i == len(W_list)-1:
                ...
                expr = expr * W
                expr = apply_activation(expr, funcs, n_double=n_double)
                # saved_expr.append(expr)
                expr = sym.BlockMatrix([[expr, saved_expr[-1]]])
            saved_expr.append(expr)
    # expr = expr * W_list[-1]
    return expr
...
def network(weights, funcs, var_names, threshold=1e-5, skip_connections=True):
    ...
    expr = sym_pp(weights[:-1], funcs, var_names,
                  threshold=threshold, n_double=n_double, skip_connections=skip_connections)
    expr = last_pp(expr, weights[-1])
    expr = expr[0, 0]
    return expr


def filter_mat(mat, threshold=1e-5):
...
def filter_expr(expr, threshold=1e-5):
...
def filter_expr2(expr, threshold=1e-5):
